function scores = nQueensFitness( populationIn )
...
% AUTHOR: PHELAN VENDEVILLE
    ...
    rows = length(populationIn(:,1)); %how many rows in the population
    scores = zeros(rows, 1); %an array to store each score in
    
    %LOOP through all of the population, calculate the fitness, and add to
    %the scores array
    j = 1;
    for i = 1:rows
        vectorOfQueens = populationIn(j,:);
